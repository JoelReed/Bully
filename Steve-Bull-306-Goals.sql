...
-- Update our columns with our preferred naming conventions
 ...
 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Date',
           'date',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Season',
           'season',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Competition',
           'competition',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].H_A',
           'home_or_away',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].W_D_L',
           'win_draw_lose',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Opponent',
           'opponent',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Result',
           'match_result',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Number_of_Goals',
           'goals_scored',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Attendance',
           'attendance',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Game_Num',
           'game_id',
           'COLUMN';

 sp_rename '[Steve Bull 306].[dbo].[Steve-Bull-306].Radius',
           'radius',
           'COLUMN';

 --Remove columns we don't need

ALTER TABLE [Steve Bull 306].[dbo].[Steve-Bull-306]
DROP COLUMN Game_Order;
...
SELECT COUNT(*) AS one_nil_win
FROM [Steve Bull 306].[dbo].[Steve-Bull-306]
WHERE (match_result = '1-0')
  OR (match_result = '0-1');

 -- Number of goals scored across different match results (W/D/L)

SELECT win_draw_lose,
       SUM(goals_scored) AS total_goals,
       ROUND(100.0 * SUM(goals_scored) /
               (SELECT SUM(goals_scored)
                FROM [Steve Bull 306].[dbo].[Steve-Bull-306]), 2) AS percentage_of_goals
FROM [Steve Bull 306].[dbo].[Steve-Bull-306]
...
SELECT CASE
       ...
       ROUND(100.0 * COUNT(*) /
               (SELECT COUNT(*)
                FROM [Steve Bull 306].[dbo].[Steve-Bull-306]), 2) AS game_percentage,
       ROUND(100.0 * SUM(goals_scored) /
               (SELECT SUM(goals_scored)
                FROM [Steve Bull 306].[dbo].[Steve-Bull-306]), 2) AS goal_percentage
FROM [Steve Bull 306].[dbo].[Steve-Bull-306]
GROUP BY CASE
             WHEN competition LIKE '%division%' THEN 'League'
             ELSE 'Cups/Playoffs'
